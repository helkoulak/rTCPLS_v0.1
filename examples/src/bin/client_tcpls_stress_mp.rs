...
impl TlsClient {
    ...
    fn handle_event(&mut self, ev: &mio::event::Event, recv_map: &mut RecvBufMap) {
        ...
        if ev.is_readable() {
                ...
                .is_handshaking()
            {
                ...
                if self.tcpls_session.tcp_connections.len() == 3 && !self.data_sent {
                    ...
                    self.tcpls_session
                        .send_on_connection(None, Some(id_set))
    ...
    fn do_write(&mut self, id: u64) {
        if self.tcpls_session.tcp_connections.contains_key(&id) {
            self.tcpls_session
                ...
                .expect("Send on connection failed");
        }
    }
    ...
    pub(crate) fn join_outstanding(&mut self, id: u64) {
        self.tcpls_session
            .join_tcp_connection(id)
            .expect("sending join request failed");
    }
...
fn main() {
    ...
    client
        .tcpls_session
        .tcpls_connect(dest_add1, Some(config), Some(server_name), false);
    // Create second tcp conection
    client
        .tcpls_session
        .tcpls_connect(dest_add2, None, None, false);
    // Create third tcp conection
    client
        .tcpls_session
        .tcpls_connect(dest_add3, None, None, false);
