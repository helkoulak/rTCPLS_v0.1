...
pub(crate) fn process_received(
    ...
    pipe.sess
        ...
        .unwrap();
    if app_bufs.get(1u16 as u32).unwrap().complete {
        return;
    } else {
        panic!("Stream was not fully received")
    }
}
fn criterion_benchmark(c: &mut Criterion<CPUTime>) {
    ...
    group.bench_with_input(
        BenchmarkId::new("Data_recv_single_stream_single_connection", data_len),
        &sendbuf,
        |b, sendbuf| {
            b.iter_batched_ref(
                || {
                    ...
                    tcpls_client
                        .stream_send(1, sendbuf.as_slice())
