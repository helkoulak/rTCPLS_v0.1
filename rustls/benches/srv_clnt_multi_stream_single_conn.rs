...
fn criterion_benchmark(c: &mut Criterion<CPUTime>) {
    ...
    group.bench_with_input(BenchmarkId::new("Data_recv_multi_stream_single_connection", data_len * 2), &sendbuf,
                           |b, sendbuf| {
                               ...
                               b.iter_batched_ref(|| {
                                   ...
                                   tcpls_client.stream_send(2, sendbuf.as_slice(), false).expect("Buffering in send buffer failed");
                                   ...
                                   while tcpls_client.tls_conn.as_mut().unwrap().wants_write(Some(2)) {
                                       sent += tcpls_client.tls_conn.as_mut().unwrap().write_tls(&mut pipe, 2).unwrap();
                                   };

                                   // Create app receive buffer
                                   recv_svr.get_or_create(1, Some( 1024 * 1024 * 1024));
                                   recv_svr.get_or_create(2, Some( 1024 * 1024 * 1024));
                                   (pipe, recv_svr)
                               },
