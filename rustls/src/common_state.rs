...
use crate::msgs::message::{
    Message, OutboundChunks, OutboundOpaqueMessage, OutboundPlainMessage, PlainMessage,
};
...
impl CommonState {
    ...
    pub(crate) fn write_plaintext(
        ...
        self.check_required_size(
            outgoing_tls,
            self.queued_key_update_message.as_deref(),
            fragments,
        )?;
    ...
    fn send_appdata_encrypt(
        ...
        for m in iter {
            ...
            let finish = if count == 0 { fin } else { false };
    ...
    fn send_single_fragment(&mut self, m: OutboundPlainMessage, id: u32, fin: bool) {
        ...
        let stream_frame_header = match m.typ {
            ApplicationData => Some(Frame::Stream {
        ...
        let em = self
            .record_layer
            .encrypt_outgoing_tcpls(m, &tcpls_header, stream_frame_header);
        self.queue_message(em.encode(), id, Some(&tcpls_header), typ);
    }

    pub(crate) fn send_single_probe(
        &mut self,
        m: OutboundPlainMessage,
    ) -> Option<OutboundOpaqueMessage> {
        // set id of stream to decide on crypto context and record seq space
        ...
        Some(
            self.record_layer
                .encrypt_outgoing_tcpls(m, &tcpls_header, None),
        )
    }
    ...
    pub(crate) fn send_ack(
        ...
        .encode(&mut b)
        .expect("encoding ack frame failed");
        // set id of stream to decide on crypto context and record seq space
        self.record_layer.encrypt_for_stream(DEFAULT_STREAM_ID);
        // Close connection once we start to run out of
        // sequence space.
        ...
        Some(self.record_layer.encrypt_outgoing_tcpls(
...
impl CommonState {
    ...
    pub(crate) fn queue_message(
        ...
        self.record_layer
            ...
            .unwrap()
            .send
            .append(msg, tcpls_header, data_type);
