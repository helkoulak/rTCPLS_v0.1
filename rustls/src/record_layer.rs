...
use crate::crypto::cipherx::{
    HeaderProtector, InboundOpaqueMessage, MessageDecrypter, MessageEncrypter,
};
...
impl RecordLayer {
    ...
    pub(crate) fn decrypt_incoming<'a>(
        ...
        match self.message_decrypter.decrypt(encr, 0) {
            Ok(plaintext) => {
    ...
    pub(crate) fn decrypt_incoming_tcpls<'a>(
        ...
        let encrypted_len = encr.payload.len();
        match self.message_decrypter.decrypt_tcpls(
            encr,
            app_bufs,
            header_decrypted,
            self.header_decrypter.as_mut().unwrap(),
        ) {
            ...
            Err(Error::DecryptError) if self.doing_trial_decryption(encrypted_len) => {
                Err(Error::General(
                    "Dropping undecryptable message after aborted early_data".to_string(),
                ))?
            }
            Err(err) => Err(err),
        }
    ...
    pub(crate) fn encrypt_outgoing_tcpls(
        ...
        self.message_encrypter
            .encrypt_tcpls(
                ...
                tcpls_header,
                frame_header,
                self.header_encrypter.as_mut().unwrap(),
            )
            .unwrap()
    }
    ...
    pub fn decrypt_header(&mut self, input: &[u8], header: &[u8]) -> Result<[u8; 8], Error> {
        self.header_decrypter
            .as_mut()
            .unwrap()
            .decrypt_in_output(input, header)
    }
    ...
    pub(crate) fn wants_close_before_encrypt(&mut self) -> bool {
        self.message_encrypter
            .get_or_create_write_seq(self.stream_in_use)
            == SEQ_SOFT_LIMIT
    }
